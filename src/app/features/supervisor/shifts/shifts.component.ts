...
export class ShiftsComponent implements OnInit, AfterViewInit {
  ...
  private readonly endpoint   = `${this.apiUrl}/api/supervisor/shifts`;
    ...
    if (event.action.key === 'delete') {
      ...
      this.http.delete(`${this.endpoint}/${item.id}`).subscribe({
        next: () => {
          ...
          this.tableSuccessMessage.set(`Shift "${item.name}" deleted.`);
        ...
        error: (err) => {
          ...
          this.errorMessage.set(err.error?.message ?? 'Failed to delete shift. Please try again.');
        },
      });
    }
  }
  ...
  onSubmit(): void {
    ...
    this.http.post<ShiftItem>(this.endpoint, body).subscribe({
      next: (item) => {
        ...
        this.closeModal();
        this.tableSuccessMessage.set(`Shift "${item.name}" created.`);
      ...
      error: (err) => {
        ...
        this.errorMessage.set(
          err.status === 409
            ...
            : (err.error?.message ?? 'Failed to create shift. Please try again.')
        );
      },
    });
  }
