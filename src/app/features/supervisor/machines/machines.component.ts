...
@Component({
  selector: 'app-supervisor-machines',
...
export class MachinesComponent implements OnInit, AfterViewInit {
  ...
  private readonly endpoint   = `${this.apiUrl}/api/supervisor/machines`;
    ...
    if (event.action.key === 'delete') {
      ...
      this.http.delete(`${this.endpoint}/${item.id}`).subscribe({
        next: () => {
          ...
          this.tableSuccessMessage.set(`Machine "${item.name}" deleted.`);
        ...
        error: (err) => {
          ...
          this.errorMessage.set(err.error?.message ?? 'Failed to delete machine. Please try again.');
        },
      });
    }
  }
  ...
  onSubmit(): void {
    ...
    this.http.post<CatalogItem>(this.endpoint, body).subscribe({
      next: (item) => {
        ...
        this.closeModal();
        this.tableSuccessMessage.set(`Machine "${item.name}" created.`);
      ...
      error: (err) => {
        ...
        this.errorMessage.set(
          err.status === 409
            ...
            : (err.error?.message ?? 'Failed to create machine. Please try again.')
        );
      },
    });
  }
